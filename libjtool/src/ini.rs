...
pub unsafe extern "C" fn ini_read_string(section: *const c_char, key: *const c_char, default: *const c_char) -> *const c_char
{
    ...
    (
        ...
        {
            let f = f.borrow_mut();
            let result = f.get_from(Some(CStr::from_ptr(section).to_str().unwrap()), CStr::from_ptr(key).to_str().unwrap())
...
#[no_mangle]
pub unsafe extern "C" fn ini_read_real(section: *const c_char, key: *const c_char, default: f64) -> f64
{
    INI_G.with
    (
        |f|
...
#[no_mangle]
pub unsafe extern "C" fn ini_write_real(section: *const c_char, key: *const c_char, value: f64) -> f64
{
    INI_G.with
    (
        |f|
