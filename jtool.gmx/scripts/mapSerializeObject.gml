/* Returns a 5-character string representation of an object
   to be used when saving that object in a map file.
   The format is 'ixxyy', where i is the palette id of the object,
   x and y are the its position shifted +128 pixels so they are positive.
   
   If the object is not an official supported object (e.g. a custom
   added palette object) then this script returns the empty string ''.
*/
...
return intToBase32String(pal_id)
     + padStringLeft(intToBase32String(obj.x+128),2,'0')
     + padStringLeft(intToBase32String(obj.y+128),2,'0')
